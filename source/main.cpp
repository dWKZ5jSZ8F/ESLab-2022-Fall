...
const static char DEVICE_NAME[] = "HR/M/Button";

static events::EventQueue event_queue(/* event count */ 16 * EVENTS_EVENT_SIZE);

class HRMButtonDemo : ble::Gap::EventHandler {
public:
    HRMButtonDemo(BLE &ble, events::EventQueue &event_queue) :
        _ble(ble),
        ...
        _hr_data_builder(_adv_buffer),
        _button(BLE_BUTTON_PIN_NAME),
        _button_service(NULL),
        _button_uuid(ButtonService::BUTTON_SERVICE_UUID),
        _button_data_builder(_adv_buffer)
    {
    }

    void start()
    {
        _ble.init(this, &HRMButtonDemo::on_init_complete);
        BSP_MAGNETO_Init(); 
...
private:
    ...
    void on_init_complete(BLE::InitializationCompleteCallbackContext *params)
    {
        ...
        _button.fall(Callback<void()>(this, &HRMButtonDemo::button_pressed));
        _button.rise(Callback<void()>(this, &HRMButtonDemo::button_released));
    ...
    void start_advertising()
    {
        ...
        _hr_data_builder.setFlags();
        _hr_data_builder.setAppearance(ble::adv_data_appearance_t::GENERIC_HEART_RATE_SENSOR);
        _hr_data_builder.setLocalServiceList({&_heartrate_uuid, 1});

        _button_data_builder.setFlags();
        _button_data_builder.setLocalServiceList(mbed::make_Span(&_button_uuid, 1));
        _button_data_builder.setName(DEVICE_NAME);
        ...
        error = _ble.gap().setAdvertisingPayload(
            ble::LEGACY_ADVERTISING_HANDLE,
            _hr_data_builder.getAdvertisingData()
        );

        if (error) {
            print_error(error, "(Heartrate) _ble.gap().setAdvertisingPayload() failed\r\n");
            return;
        }

        error = _ble.gap().setAdvertisingPayload(
            ble::LEGACY_ADVERTISING_HANDLE,
            _button_data_builder.getAdvertisingData()
        );

        if (error) {
            print_error(error, "(Button) _ble.gap().setAdvertisingPayload() failed");
            return;
        }
        ...
        printf("HR/M/Button service advertising, please connect\r\n");
    }
...
private:
    ...
    ble::AdvertisingDataBuilder _hr_data_builder;
    ble::AdvertisingDataBuilder _button_data_builder;
};
...
int main()
{
    ...
    HRMButtonDemo demo(ble, event_queue);
    demo.start();
